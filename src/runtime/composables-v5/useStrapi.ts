...
interface StrapiV5Client<T> {
  ...
  findOne<F = T>(contentType: string, documentId: string, params?: Omit<Strapi5RequestParams<F>, 'filters'>, fetchOptions?: FetchOptions): Promise<Strapi5ResponseSingle<F>>
  create<F = T>(contentType: string, data: Partial<F>, params?: Omit<Strapi5RequestParams<F>, 'filters'>): Promise<Strapi5ResponseSingle<F>>
  update<F = T>(contentType: string, documentId: string | Partial<F>, data?: Partial<F>, params?: Omit<Strapi5RequestParams<F>, 'filters'>): Promise<Strapi5ResponseSingle<F>>
  delete(contentType: string, documentId?: string, params?: Pick<Strapi5RequestParams<never>, 'locale'>): Promise<void>
}
...
export const useStrapi = <T>(): StrapiV5Client<T> => {
   ...
   * @param  {Omit<Strapi5RequestParams<T>, 'filters'>} [params] - Query parameters
   * @returns Promise<T>
   */
  const findOne = <T>(contentType: string, documentId: string, params?: Omit<Strapi5RequestParams<T>, 'filters'>, fetchOptions?: FetchOptions): Promise<Strapi5ResponseSingle<T>> => {
   ...
   * @returns Promise<T>
   */
  const create = <T>(contentType: string, data: Partial<T>, params: Omit<Strapi5RequestParams<T>, 'filters'> = {}): Promise<Strapi5ResponseSingle<T>> => {
   ...
   * @returns Promise<T>
   */
  const update = <T>(contentType: string, documentId: string | Partial<T>, data?: Partial<T>, params: Omit<Strapi5RequestParams<T>, 'filters'> = {}): Promise<Strapi5ResponseSingle<T>> => {
  ...
  const _delete = (contentType: string, documentId?: string, params?: Pick<Strapi5RequestParams<never>, 'locale'>): Promise<void> => {
    ...
    return client(path, { method: 'DELETE', params })
  }

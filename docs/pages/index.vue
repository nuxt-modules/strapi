...
<template>
  <div>
    ...
    <ULandingSection :title="page.features.title">
      <UPageGrid>
        <ULandingCard v-for="(item, index) of page.features.items" :key="index" :icon="item.icon" :title="item.title">
          <MDC :value="item.description" class="prose prose-primary dark:prose-invert max-w-none" />
        </ULandingCard>
      </UPageGrid>
    </ULandingSection>
  </div>

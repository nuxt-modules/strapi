...
This action will confirm the email of a user.
...
```vue
<script setup lang="ts">
const { emailConfirmation } = useStrapiAuth()
...
- **Arguments:**
  - data: [`StrapiSendEmailConfirmationData`](https://github.com/nuxt-community/strapi-module/blob/dev/src/runtime/types/index.d.ts#L555)
- **Returns:** `Promise<void>`
